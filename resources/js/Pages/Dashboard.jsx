...
export default function Dashboard({ auth, totalPendingTasks, myPendingTasks, myProgressTasks, totalProgressTasks, myCompletedTasks, totalCompletedTasks, activeTasks }) {
    return (
        <AuthenticatedLayout
            user={auth.user}
                                ...
                                <tbody>
                                    {activeTasks.data.map(task => (
                                        <tr key={task.id}>
                                            ...
                                            <td className='px-3 py-2'><span className={'px-2 py-1 rounded text-nowrap text-white ' + TASK_STATUS_CLASS_MAP[task.status]}>{TASK_STATUS_TEXT_MAP[task.status]}</span></td>
