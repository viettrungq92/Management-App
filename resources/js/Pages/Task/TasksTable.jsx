                ...
                <table className='w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400'>
                    ...
                    <tbody>
                        {tasks.data.map((tasks) => (
                            <tr className='bg-white border-b dark:bg-gray-800 dark:border-gray-700' key={tasks.id}>
                                <td className='px-3 py-2'>{tasks.id}</td>
                                <td className='px-3 py-2'>
                                    <img src={tasks.image_path} style={{ width: 60 }} />
                                </td>
                                {!hideProjectColumn && (<td className='px-3 py-2'>{tasks.project.name}</td>)}
                                <th className='px-3 py-2 text-gray-100 hover:underline'>
                                    <Link href={route('task.show', tasks.id)}>
                                        {tasks.name}
                                    </Link>
                                </th>
                                <td className='px-3 py-2'><span className={'px-2 py-1 rounded text-nowrap text-white ' + TASK_STATUS_CLASS_MAP[tasks.status]}>{TASK_STATUS_TEXT_MAP[tasks.status]}</span></td>
                                <td className='px-3 py-2 text-nowrap'>{tasks.created_at}</td>
                                <td className='px-3 py-2 text-nowrap'>{tasks.due_date}</td>
                                <td className='px-3 py-2'>{tasks.createdBy.name}</td>
                                <td className='px-3 py-2 text-nowrap'>
                                    <Link href={route('task.edit', tasks.id)} className='font-medium text-blue-600 dark:text-blue-500 hover:underline mx-1'>
                                        Edit
                                    </Link>
                                    <button onClick={(e) => deleteTask(tasks)} className='font-medium text-red-600 dark:text-red-500 hover:underline mx-1'>
                                        Delete
                                    </button>
                                </td>
                            </tr>
